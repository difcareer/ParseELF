...
public class ElfType32 {
    ...
    /**
     * value为 section 的name,是shstrtab中的偏移量
     */
    public Map<List<elf32_strtb>, Integer> strtbs = new HashMap<>();

    /**
     * 单独存放
     */
    public byte[] shstrtab;

    /**
     * value为 section 的name,是shstrtab中的偏移量
     */
    public Map<List<Elf32_Sym>, Integer> symtbs = new HashMap<>();
    ...
    public class elf32_rela {
        ...
        public String toString() {
            return "r_offset:" + Utils.bytes2HexString(r_offset) + ";r_info:" + Utils.bytes2HexString(r_info)
                    + ";r_addend:" + Utils.bytes2HexString(r_info);
        }
    }
    ...
    public void printPhdrList() {
        for (int i = 0; i < phdrList.size(); i++) {
            System.out.println();
            System.out.println("The " + i + " Program Item:");
    ...
    public static class elf32_shdr {
        ...
        @Override
        public String toString() {
            return "sh_name:" + Utils.bytes2HexString(sh_name) + " txt: "
                    + ParseSo.getStrFromShStrTab(Utils.byte2Int(sh_name)) + "\nsh_type:"
                    + Utils.bytes2HexString(sh_type) + "\nsh_flags:" + Utils.bytes2HexString(sh_flags) + "\nsh_add:"
                    + Utils.bytes2HexString(sh_addr) + "\nsh_offset:" + Utils.bytes2HexString(sh_offset) + "\nsh_size:"
                    + Utils.bytes2HexString(sh_size) + "\nsh_link:" + Utils.bytes2HexString(sh_link) + "\nsh_info:"
                    + Utils.bytes2HexString(sh_info) + "\nsh_addralign:" + Utils.bytes2HexString(sh_addralign)
    ...
    public void printShdrList() {
        for (int i = 0; i < shdrList.size(); i++) {
            System.out.println();
            System.out.println("The " + i + " Section Item:");
    ...
    public static class elf32_strtb {
        ...
        @Override
        public String toString() {
            return "str_name:" + Utils.bytes2HexString(str_name) + "len:" + len + " txt:" + new String(str_name);
        }
    }
}
